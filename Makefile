...
env:
	test -d venv || python3.7 pyvenvex.py venv

# Install requirements_dev
dev: env
	test ! -d venv || venv/bin/pip install -r requirements_dev.txt

# Install GOApy
install-in-venv: env
	test ! -d venv || venv/bin/python setup.py install

# upload tag
upload: env
	test ! -d venv || venv/bin/python setup.py upload
...
# Clean all
clean: clean-pyc
	test ! -d venv || rm --force --recursive venv
	test ! -d dist || rm --force --recursive dist
	test ! -d build || rm --force --recursive build
	test ! -d goap.egg-info || rm --force --recursive goap.egg-info
	test ! -d .pytest_cache || rm --force --recursive .pytest_cache
	test ! -d docs/_build || rm --force --recursive  docs/_build
	test ! -d .tox/ || rm --force --recursive .tox
